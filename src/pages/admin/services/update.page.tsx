...
export default function UpdateServicesForm({ id }: { id: string }) {
  const [state, action, pending] = useActionState(updateServices, null);
  ...
  useEffect(() => {
    const fetchServices = async () => {
      const res = await getSpecificService(id);
      if (!res.status) {
  ...
  useEffect(() => {
    if (state?.status === true) {
      ...
      router.push("/dashboard/admin/bussines/services/");
        ...
        <form action={action}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select name="item_type_id" value={services?.item_type_id || ""}>
                <SelectTrigger>
                  <SelectValue
                ...
                <SelectContent defaultValue={services?.item_type_id || ""}>
                  {itemType.map((val) => (
                    <SelectItem
          ...
          <div className="grid grid-cols-2 gap-3 mt-5">
            <div>
              ...
              <Input
                ...
                className="w-full border-input placeholder:font-normal font-semibold shadow-none focus:ring-1 focus:ring-ring focus:outline-none focus:border-ring"
                type="number"
                defaultValue={services?.estimated_hours || ""}
              />
